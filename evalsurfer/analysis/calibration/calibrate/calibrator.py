"""The judge calibrator -- the "eval of the eval" service.

:class:`Calibrator` scores one judge-produced report against a
:class:`~evalsurfer.analysis.calibration.calibrate.models.CalibrationCase`
...
(:meth:`~Calibrator.summarize`). Deterministic, standard library only, no model
...
class Calibrator:
    ...
    #: Ids of the criteria that live on the safety category, taken from the
    ...
    def _has_critical_safety_issue(cls, report: Mapping[str, Any]) -> bool:
        ...
        whose ``criterion_id`` names one of the safety-category criteria; a
    ...
    def check_report(
        ...
        ``plan_match`` (the planner's applicable categories for ``case.signals``
        equal ``case.expected_applicable_categories``), ``decision_match`` (the
        ...
        plan_match = (
            cls._applicable_categories(case.signals)
            == case.expected_applicable_categories
        )
