"""The pipeline failure-map service and its rendering literals.

:class:`FailureMap` projects a produced report onto the application pipeline
(Prompt -> Retriever -> Ranker -> Generator -> Tool -> Response) and marks which
stages are weak, rendering the result as a structured list, a Mermaid
``flowchart LR``, and a plain arrow chain. The ``StageDiagnosis`` value object it
emits lives in :mod:`evalsurfer.analysis.diagnostics.failure_map.models`. Standard
library only, no model calls; it never mutates the report.
"""
...
# Each criterion id mapped to its ``(category, group)`` location, from the planner's
# canonical catalog. Used to decide which pipeline stage a criterion feeds.
_CRITERION_LOCATION: dict[str, tuple[str, str | None]] = {
    criterion.id: (criterion.category, criterion.group)
...
class FailureMap:
    ...
    def _response_status(cls, report: Mapping[str, Any]) -> str:
        ...
        A ``fail`` decision reads as ``fail``; both ``pass`` and
        ``pass_with_fixes`` read as ``ok`` -- a conditional pass is still a pass,
        and the stages that need fixes are already flagged ``fail`` by their own
        criteria, so marking Response ``fail`` for ``pass_with_fixes`` would
        overstate severity. A missing decision reads as ``na``.
        ...
        if decision is None:
            return constants.STAGE_STATUS_NA
        if decision == constants.DECISION_FAIL:
