"""The maturity classifier service and its gating logic.

:class:`MaturityClassifier` places an evaluation target on the six-level maturity
ladder from the same :class:`Signals` the planner infers, plus explicit
``multi_agent`` / ``self_improving`` flags. The ``MaturityLevel`` value object it
resolves to lives in :mod:`evalsurfer.analysis.diagnostics.maturity.models`.
Deterministic: no model calls, no third-party dependencies; it never mutates its
inputs.
"""
