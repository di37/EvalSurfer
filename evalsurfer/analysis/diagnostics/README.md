# `evalsurfer/analysis/diagnostics/` — explain and compare, don't just score
...
> **As MCP tools:** the harness LLM calls these directly via the `evalsurfer[mcp]` server — `explain`, `root_cause`, `regression_diff`, `maturity`, `industry_profiles`, `industry_profile`, `review_gate`, `personas`, `failure_map`, `diagnose`, `golden_set`, `build_evidence`. See [`../mcp/`](../../interface/mcp/) and [`../../docs/mcp.md`](../../../docs/mcp.md).
