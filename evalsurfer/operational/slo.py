...
Where :mod:`evalsurfer.operational.metrics` turns request traces into raw
...
class OperationalScore:
    """The operational pillar's SLO auto-scoring result."""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Returns:
            ``{"criteria": [...], "pillar_score": float | None}`` where each
...
class OperationalScorer:
    ...
    OPERATIONAL_CRITERIA: tuple[Criterion, ...] = tuple(
        ...
        if criterion.pillar == constants.PILLAR_OPERATIONAL
    )
    ...
    def score(self, traces_payload: Any) -> dict[str, Any]:
        """Auto-score the operational pillar from request traces against the SLO.
        ...
        Returns:
            ...
            "pillar_score": float | None}``. ``token_efficiency`` has no SLO and
        ...
        pillar_score = ScoringModel.pillar_score(
            [criterion.score for criterion in criteria]
        )
        return OperationalScore(criteria=criteria, pillar_score=pillar_score).to_dict()
