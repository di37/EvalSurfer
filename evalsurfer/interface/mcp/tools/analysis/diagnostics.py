...
def root_cause(report: m.Report) -> dict:
    """Attribute lost points by category and criterion group, with a top contributor."""
...
@mcp.tool()
def industry_profile(profile: str, category_scores: m.CategoryScores) -> dict:
    """Re-weight the overall score using an industry profile's category weights."""
    weighted = IndustryProfiler(profile).weighted_overall(category_scores.model_dump())
...
def diagnose(
    ...
    return DiagnosticsBundle.run(
        ...
        signals=Signals(**signals.model_dump()) if signals else None,
    )
