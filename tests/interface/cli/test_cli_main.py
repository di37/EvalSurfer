...
def run_json(argv: list[str]) -> tuple[int, dict[str, Any]]:
    ...
    if not isinstance(result, dict):
        raise TypeError("expected CLI JSON output to be an object")
    return code, result
...
class CliMainTest(unittest.TestCase):
    def test_evaluate_validate_gate_flow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            sample = _write(
                tmp,
                "s.json",
                {"sample": {"answer": "hi"}, "scores": {"correctness_accuracy": 5}},
            )
    ...
    def test_validate_rejects_bad_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            bad = _write(
                tmp, "bad.json", {"overall": {"score": 99, "decision": "great"}}
            )
    ...
    def test_gate_below_minimum_exits_one(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            report = _write(
                ...
                "r.json",
                {
                    "overall": {"score": 6.0, "decision": "pass_with_fixes"},
                    "metrics": {},
                    ...
                    "traces": [
                        {
                            ...
                            "response_completed_at": "2026-07-08T12:00:01Z",
                        }
                    ]
                },
            )
    ...
    def test_diagnose_adds_maturity_from_signals_or_sample(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            request = _write(
                tmp,
                "request.json",
                {
                    "sample": {"answer": "hi"},
                    "scores": {"correctness_accuracy": 5},
                },
            )
            report = os.path.join(tmp, "report.json")
            self.assertEqual(run(["evaluate", request, "--out", report]), 0)
            ...
            code, diagnostics = run_json(["diagnose", report, "--signals", inferred])
            self.assertEqual(code, 0)
            self.assertEqual(diagnostics["maturity"]["level"], 2)
            ...
            code, diagnostics = run_json(["diagnose", report, "--signals", explicit])
            self.assertEqual(code, 0)
            self.assertEqual(diagnostics["maturity"]["level"], 3)

            code, diagnostics = run_json(["diagnose", report])
            self.assertEqual(code, 0)
            self.assertNotIn("maturity", diagnostics)

            invalid = _write(
                tmp,
                "invalid-signals.json",
                {"signals": {"tool_calls": "false"}},
            )
            self.assertEqual(run(["diagnose", report, "--signals", invalid]), 1)

            typo = _write(
                tmp,
                "typo-signals.json",
                {"signals": {"tool_calls": True, "tool_calss": True}},
            )
            self.assertEqual(run(["diagnose", report, "--signals", typo]), 1)

            boolean_sample = _write(
                tmp,
                "boolean-sample.json",
                {"answer": True, "expected": True},
            )
            code, diagnostics = run_json(
                ["diagnose", report, "--signals", boolean_sample]
            )
            self.assertEqual(code, 0)
            self.assertIn("maturity", diagnostics)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            case = _write(
                ...
                "c.json",
                {
                    ...
                    "expected_applicable_categories": ["quality", "safety"],
