...
class PillarDeltaTest(unittest.TestCase):
    def test_pillar_deltas(self) -> None:
        pillars = RegressionDiffer.diff(BEFORE, AFTER)["pillars"]
        self.assertEqual(
            pillars[constants.PILLAR_QUALITY], {"before": 6.5, "after": 8.0, "delta": 1.5}
        )
        self.assertEqual(
            pillars[constants.PILLAR_SAFETY], {"before": 9.0, "after": 8.0, "delta": -1.0}
        )
        self.assertEqual(
            pillars[constants.PILLAR_OPERATIONAL], {"before": 7.0, "after": 7.0, "delta": 0.0}
        )

    def test_all_three_pillars_always_present(self) -> None:
        pillars = RegressionDiffer.diff({}, {})["pillars"]
        self.assertEqual(set(pillars), set(constants.PILLARS))
        for entry in pillars.values():
            self.assertEqual(entry, {"before": None, "after": None, "delta": None})

    def test_pillar_delta_none_when_one_score_missing(self) -> None:
        before = {"pillars": {constants.PILLAR_QUALITY: {"score": 6.0, "criteria": []}}}
        after = {"pillars": {constants.PILLAR_QUALITY: {"score": None, "criteria": []}}}
        self.assertIsNone(
            RegressionDiffer.diff(before, after)["pillars"][constants.PILLAR_QUALITY]["delta"]
        )
...
class CriteriaDiffTest(unittest.TestCase):
    ...
    def test_none_in_both_is_unchanged(self) -> None:
        before = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [_criterion("x", "X", None)]}}}
        after = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [_criterion("x", "X", None)]}}}
    ...
    def test_name_falls_back_to_before_then_id(self) -> None:
        before = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [_criterion("x", "Named", 3)]}}}
        after = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [{"id": "x", "score": 4}]}}}
        self.assertEqual(_by_id(RegressionDiffer.diff(before, after))["x"]["name"], "Named")
        # When neither report names it, fall back to the id.
        bare = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [{"id": "y", "score": 3}]}}}
        self.assertEqual(_by_id(RegressionDiffer.diff(bare, bare))["y"]["name"], "y")

    def test_criteria_matched_within_same_pillar(self) -> None:
        # The same id in different pillars must not be matched across pillars.
        before = {"pillars": {constants.PILLAR_QUALITY: {"criteria": [_criterion("shared", "Q", 3)]}}}
        after = {"pillars": {constants.PILLAR_SAFETY: {"criteria": [_criterion("shared", "S", 5)]}}}
    ...
    def test_criteria_ordering_is_pillar_then_before_then_new(self) -> None:
...
class IdenticalAndEmptyTest(unittest.TestCase):
    ...
    def test_missing_pillars_key_is_tolerated(self) -> None:
