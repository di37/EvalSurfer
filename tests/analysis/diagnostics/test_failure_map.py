...
def _report(criteria_by_category: dict | None = None, **extra: object) -> dict:
    report: dict = {}
    if criteria_by_category is not None:
        for category, crits in criteria_by_category.items():
...
class FailureMapShapeTest(unittest.TestCase):
    ...
    def test_stage_criteria_mapping(self) -> None:
        ...
        # Generator covers generation quality, multi-turn, and the whole safety category.
...
class FailureMapEmptyTest(unittest.TestCase):
    ...
    def test_missing_categories_key_is_all_na(self) -> None:
        result = _render({"metadata": {"model": "x"}})
        self.assertTrue(
            all(s["status"] == constants.STAGE_STATUS_NA for s in result["stages"])
        )

    def test_non_mapping_layer_tolerated(self) -> None:
        result = _render({"metrics": ["not", "a", "mapping"]})
        self.assertTrue(
            all(s["status"] == constants.STAGE_STATUS_NA for s in result["stages"])
        )
...
class FailureMapDiagnosisTest(unittest.TestCase):
    ...
    def test_generation_failure_hits_generator(self) -> None:
...
class FailureMapScoreHandlingTest(unittest.TestCase):
    ...
    def test_missing_id_is_skipped(self) -> None:
        report = {"metrics": {"quality": {"criteria": [{"score": 1}]}}}
        result = _render(report)
        self.assertTrue(
            all(s["status"] == constants.STAGE_STATUS_NA for s in result["stages"])
        )
...
class FailureMapResponseTest(unittest.TestCase):
    ...
    def test_response_fail_only_on_fail(self) -> None:
    ...
    def test_response_ok_on_pass_with_fixes(self) -> None:
        # A conditional pass is still a pass; weak stages are already flagged by
        # their own criteria, so Response must not overstate it as a failure.
        stages = _stages(
