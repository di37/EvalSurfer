...
class GoldenSetExpectedValuesTest(unittest.TestCase):
    ...
    def test_clean_rag_case_expects_pass(self) -> None:
        ...
        self.assertEqual(case.expected_applicable_categories, frozenset({"quality", "safety"}))
    ...
    def test_operational_only_case_skips_safety_and_quality(self) -> None:
        ...
        self.assertEqual(case.expected_applicable_categories, frozenset({"operational"}))
        self.assertEqual(case.expected_decision, "pass_with_fixes")
...
class GoldenSetNonTautologicalTest(unittest.TestCase):
    ...
    def test_wrong_expected_categories_is_flagged(self) -> None:
        base = _cases_by_name()["clean_rag_pass"]
        broken = replace(base, expected_applicable_categories=frozenset({"operational"}))
        result = GoldenSet.run_case(broken)
        self.assertFalse(result["ok"])
        self.assertTrue(any("applicable categories" in f for f in result["failures"]))
    ...
    def test_both_wrong_records_two_failures(self) -> None:
        ...
        broken = replace(
            ...
            expected_applicable_categories=frozenset({"operational"}),
        )
        result = GoldenSet.run_case(broken)
        self.assertFalse(result["ok"])
...
class GoldenSetEdgeCaseTest(unittest.TestCase):
    def test_none_scores_are_excluded_from_category(self) -> None:
        ...
        case = GoldenCase(
            ...
            expected_applicable_categories=frozenset({"quality", "safety"}),
            expected_decision="pass",
        )
        self.assertTrue(GoldenSet.run_case(case)["ok"])

    def test_all_none_category_is_treated_as_unscored(self) -> None:
        # A category with only None scores yields no category score; with no safety
    ...
    def test_empty_signals_have_no_applicable_categories(self) -> None:
        case = GoldenCase(
            name="empty_signals",
            signals=Signals(),
            criterion_scores={},
...
class GoldenSetValidationTest(unittest.TestCase):
    ...
    def test_non_frozenset_expected_categories_rejected(self) -> None:
        kwargs = self._valid_kwargs()
        kwargs["expected_applicable_categories"] = {"quality"}
        with self.assertRaises(TypeError):
            GoldenCase(**kwargs)

    def test_unknown_expected_category_rejected(self) -> None:
        kwargs = self._valid_kwargs()
        kwargs["expected_applicable_categories"] = frozenset({"speed"})
        with self.assertRaises(ValueError):
            GoldenCase(**kwargs)
