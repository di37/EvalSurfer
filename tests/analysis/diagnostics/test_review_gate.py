...
def _report(criteria_by_category=None, top_issues=None):
    """Build a minimal report with the given category criteria and top issues,
    nesting each category under its metrics / assurance report section."""

    report: dict = {}
    for category_id, criteria in (criteria_by_category or {}).items():
        layer = constants.LAYER_BY_CATEGORY[category_id]
        report.setdefault(layer, {})[category_id] = {"score": None, "criteria": list(criteria)}
...
class ReviewGateTest(unittest.TestCase):
    ...
    def test_tolerates_missing_and_malformed_keys(self) -> None:
        # No categories, top_issues is not a list, stray keys present.
