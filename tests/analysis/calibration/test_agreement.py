...
class KrippendorffAlphaTest(unittest.TestCase):
    ...
    def test_no_pairable_data_returns_none(self) -> None:
        # Every unit has fewer than two valid ratings, so there is no evidence of
        # agreement: alpha is undefined (None), not a false "perfect" 1.0.
        self.assertIsNone(AgreementStats.krippendorff_alpha([[1], [2], [3]]))
        self.assertIsNone(AgreementStats.krippendorff_alpha([[1, None], [2, None]]))
