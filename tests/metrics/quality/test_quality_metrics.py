...
class TokenizeTest(unittest.TestCase):
    ...
    def test_light_stem_does_not_overstrip_ted_verbs(self) -> None:
        # Regression: a "-ted" rule wrongly stripped the stem's own "t", turning
        # "started" into "star". Past-tense "-ed" must leave the "t" intact.
        self.assertEqual(light_stem("started"), "start")
        self.assertEqual(light_stem("wanted"), "want")
        self.assertEqual(light_stem("painted"), "paint")
...
class TextMetricsTest(unittest.TestCase):
    ...
    def test_meteor_stemming_matches_ted_verbs(self) -> None:
        # With "-ted" over-stripping fixed, "started"/"start" now stem-match.
        stemmed = TextMetrics.meteor("the report started", "the report start", stem=True)
        exact_only = TextMetrics.meteor("the report started", "the report start", stem=False)
