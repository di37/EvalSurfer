...
class EvaluatorTest(unittest.TestCase):
    def test_assembles_full_report(self) -> None:
        report = Evaluator.evaluate(_RAG_REQUEST)
        self.assertIn(report["decision"], ("pass", "pass_with_fixes", "fail"))
        self.assertEqual(report["overall"]["decision"], report["decision"])
        self.assertIn("quality", report["pillars"])
        self.assertIn("safety", report["pillars"])
        self.assertIn("operational", report["pillars"])  # auto-scored from traces
        self.assertIn("coverage", report)
        # report-only diagnostics always present; maturity present (signals known)
        for key in ("explainability", "root_cause", "failure_map", "review_gate", "maturity"):
            self.assertIn(key, report["diagnostics"])

    def test_operational_pillar_is_auto_scored(self) -> None:
        report = Evaluator.evaluate(_RAG_REQUEST)
        op = report["pillars"]["operational"]
        # latency p95 was 3.0s vs 3000ms SLO -> at target -> score 3
    ...
    def test_no_traces_means_no_operational_pillar(self) -> None:
        request = {"sample": {"answer": "hi"}, "scores": {"correctness_accuracy": 5}}
        report = Evaluator.evaluate(request)
        self.assertNotIn("operational", report["pillars"])
    ...
    def test_flattens_nested_and_flat_scores(self) -> None:
        ...
        crit = nested["pillars"]["quality"]["criteria"]
    ...
    def test_critical_safety_issue_forces_fail(self) -> None:
        ...
        self.assertEqual(Evaluator.evaluate(request)["decision"], "fail")
