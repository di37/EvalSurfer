...
def _report(pillars: dict[str, list[tuple[str, int | None]]]) -> dict:
    """Build a minimal report from {pillar: [(id, score), ...]}."""
    ...
    return {
        "pillars": {
            ...
            for pillar, criteria in pillars.items()
        }
    }
...
class AttributionMathTest(unittest.TestCase):
    def test_deductions_sum_to_lost_points_multi_pillar(self) -> None:
    ...
    def test_single_pillar_denominator(self) -> None:
...
class AssessedOnlyTest(unittest.TestCase):
    ...
    def test_pillar_with_all_none_is_not_counted(self) -> None:
        report = _report(
...
class EdgeCaseTest(unittest.TestCase):
    ...
    def test_missing_criteria_key_tolerated(self) -> None:
        result = Explainer.explain({"pillars": {"quality": {}, "safety": {"criteria": []}}})
        self.assertEqual(result["deductions"], [])
        self.assertIsNone(result["overall"])

    def test_missing_name_and_id_tolerated(self) -> None:
        report = {"pillars": {"quality": {"criteria": [{"score": 3}]}}}
    ...
    def test_none_name_falls_back_to_id(self) -> None:
        report = {
            "pillars": {"quality": {"criteria": [{"id": "acc", "name": None, "score": 2}]}}
        }
...
class ValidationTest(unittest.TestCase):
    ...
    def test_non_integer_score_raises(self) -> None:
        report = {"pillars": {"quality": {"criteria": [{"id": "a", "score": "5"}]}}}
        with self.assertRaises(ValueError):
            Explainer.explain(report)

    def test_boolean_score_rejected(self) -> None:
        report = {"pillars": {"quality": {"criteria": [{"id": "a", "score": True}]}}}
        with self.assertRaises(ValueError):
            Explainer.explain(report)
...
class ImmutabilityTest(unittest.TestCase):
    ...
    def test_deduction_dataclass_is_frozen(self) -> None:
        deduction = Deduction(id="a", name="A", pillar="quality", score=3, points_lost=1.0)
