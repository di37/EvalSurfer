...
class ProfilesPresetTest(unittest.TestCase):
    ...
    def test_every_preset_covers_the_three_pillars(self) -> None:
        for name, weights in constants.INDUSTRY_PROFILES.items():
            with self.subTest(profile=name):
                self.assertEqual(set(weights), set(constants.PILLARS))
    ...
    def test_specific_preset_weights(self) -> None:
        self.assertEqual(
            constants.INDUSTRY_PROFILES["healthcare"][constants.PILLAR_SAFETY], 0.5
        )
        self.assertEqual(
            constants.INDUSTRY_PROFILES["gaming"][constants.PILLAR_OPERATIONAL], 0.5
        )
        self.assertEqual(
            constants.INDUSTRY_PROFILES["education"][constants.PILLAR_QUALITY], 0.6
        )
...
class IndustryProfilerConstructionTest(unittest.TestCase):
    def test_named_profile_exposes_a_copy_of_the_weights(self) -> None:
        ...
        profiler.weights[constants.PILLAR_SAFETY] = 0.0
        self.assertEqual(
            constants.INDUSTRY_PROFILES["healthcare"][constants.PILLAR_SAFETY], 0.5
        )
    ...
    def test_unknown_pillar_key_raises(self) -> None:
        with self.assertRaises(ValueError):
...
class WeightedOverallTest(unittest.TestCase):
    ...
    def test_none_pillar_is_dropped_and_weights_renormalise(self) -> None:
    ...
    def test_single_scored_pillar_returns_that_score(self) -> None:
    ...
    def test_missing_pillar_keys_are_ignored(self) -> None:
    ...
    def test_zero_total_weight_over_scored_pillars_raises(self) -> None:
        # the only scored pillar has zero weight in the custom profile
