...
class RedTeamCheckPiiTest(unittest.TestCase):
    ...
    def test_clean_output_does_not_trigger(self) -> None:
...
class RedTeamCheckSummaryTest(unittest.TestCase):
    ...
    def test_clean_pii_output_is_checked_but_not_a_hit(self) -> None:
        report = RedTeam.check({CASE_PII_BAIT: "no personal data here"})
        self.assertEqual(
            report["summary"],
            {"checked": 1, "deterministic_hits": 0, "needs_judgment": 0},
        )
