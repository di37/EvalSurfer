...
| --- | --- | --- |
| [`ci.yml`](ci.yml) | push to `main`, and every pull request | Installs the package with dev extras, runs the full `unittest` suite on Python 3.10 / 3.11 / 3.12, then smoke-tests the `metrics` and `plan` CLIs. |

## Notes

- The test matrix mirrors `requires-python = ">=3.10"` in [`../../pyproject.toml`](../../pyproject.toml); add a Python version here and there together.
